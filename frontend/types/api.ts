...
export interface paths {
    ...
    "/subscriptions/fib/pending": {
         ...
         * @description Recovers an in-progress payment — the QR code, manual code and app link of a DRAFT subscription that is still valid. The draft is LIVE-VERIFIED against FIB first: if it turns out to be paid, the plan is activated on the spot and the response is `data: null` with the message "Payment confirmed — your plan is now active" (exact prefix is a frontend contract). Expired DRAFTs are swept. Returns `data: null` when there is nothing to resume.
         */
        get: {
            parameters: {
                query?: never;
